...
try:
    ...
    sys.exit(sys.argv[0] + " <phage metagenome presence>  <bacterial metagenome presence> <number of stdevs below mean to include as positives. Use 0 to use the mean as the cutoff. Use -1 to ignore all 0 values in the data (and use the mean as a cutoff)>")
...
for p in phage:
    ...
    indices = [i for i, x in enumerate(mi) if x == max(mi)]
    print p + "\t" + "\t".join(map(str, [allbacteria[i] for i in indices]))
