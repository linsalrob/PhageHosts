...
The code used for identification of the hosts of different phages. This
...
should run *out of the box* on those machines, but it will also run on
...
The phage genomes were downloaded from RefSeq and parsed to get the
information in the "host" field. These data were converted to coding and
protein sequences:

For example, to convert from GBFF to FNA format for all of the open
reading frames:
...
use ``perl PhageHosts/code/get_viral_dna.pl`` to split the viruses into
...
challenges on Eukaryotic viral data sets.

Extracting the DNA and protein sequences
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

To get the phage coding sequences, we pull them out of the fasta file,
...
These were extracted and a single bacterial database was made for blastn
searches
...
We extract the NC\_ ids from the complete bacteria, above:
...
We edited this last file manually to ensure that we only have complete
bacterial genomes.

We also downloaded the gbff files and orf files from RefSeq, and then
used those to create a tbl file with both protein and CDS sequences, and
create file called protein\_list that has a list of all of the gbff
files that we extracted.
...
Based on this output we create two files, refseq\_proteins.faa (with
proteins) and refseq\_orfs.faa (with DNA) from just the complete
genomes.
...
We wrote code to automatically get the taxonomy for most of the hosts
from the RefSeq files, but there were a few that we could not map, so we
added those manually. To whit:
...
**NOTE:** *When we score the connection between phage and host, we need
...
and another file refseq2taxonomy.py which added the tax id to the list
...
Note that in this process we deleted two phages whose hosts were not
really known (NC\_000935) APSE-1 whose host was Endosymbiont and Zamilon
virophage (NC\_022990) whose host is Mont1 megavirus)

Resulting files
~~~~~~~~~~~~~~~

This resulted in one key file that we use in this work
`data/all\_host\_taxid.txt <https://github.com/linsalrob/PhageHosts/blob/master/data/all_host_taxid.txt>`__
...
In general, we are going to print out tab separated files containing the
...
phylogenetic tree to score how good our matches are.

We have a few keys pieces of code for this work:

-  NC2taxid.py is python code that converts any RefSeq ID (NC\_\\d+)
   ...
   RefSeq IDs map to host taxonomy IDs.
-  scoreTaxId.py is python code that takes the a set of taxonomy IDs and
   scores all subsequent elements in the set against the first member of
   the set, using 'species', 'genus', 'family', 'order', 'class',
   'phylum', 'superkingdom' taxonomic levels from NCBI.
...
Start by making a database of just the complete genomes protein
sequences
...
*NOTE:* There are mutliple proteins with the same ID that come from
...
solutions from the blastx output.
...
Now we just count the hosts with the most number of hits to each of the
phages, and score those hits
...
To see what would happen, we also compared the phages to all the
...
confusion from the similarity searches.

We used blast to compare the complete phage genomes against all
bacterial proteins in the GenBank nr and then gi\_taxid table of the
`NCBI Taxonomy Site <ftp://ftp.ncbi.nlm.nih.gov/pub/taxonomy/>`__ (`here
is an alternate link <http://tinyurl.com/ncbiftp>`__ as there is an
issue linking to FTP sites from github) to get the taxonomy id of the
top hits. We compared those hits with the expected hosts.

The blast is a standard blast using nr as the database, and our output
file is in blast tab separated output format (standard format with
*qlen* and *slen* added to the output). We then use

::

        python PhageHosts/code/blast2taxid.py  phage_with_host.fna.blastx prot > phage.host.blastx.taxid

to create a list of phage and host taxids. Now we have to convert these
to taxids and score the hits:
...
This creates three files, ``all_hits.score``, ``equal_hits.score``,
``best_hits.score``.

Similarity to complete genomes (blastn)
---------------------------------------

We start with direct matches between the phages and hosts using the
complete bacterial genomic DNA, first by making a database and then by
using our cluster to blast the phages against the database. *Note:* We
...
Pilercr was downloaded from `drive5 <http://www.drive5.com/pilercr/>`__
and run against all complete genomes to create the database
`data/pilercr1.06.output.fna <data/pilercr1.06.output.fna>`__.

blastn searches were performed using modified parameters that should be
better for short matches:
...
These blast searches were
`compared <PhageHosts/code/score_blast.py%20score_blast.py>`__,
`converted to
taxids <PhageHosts/code/crispr_blast2tax.py%20crispr_blast2tax.py>`__,
and `scored <PhageHosts/code/scoreTaxId.py%20scoreTaxId.py>`__:
...
To identify the longest exact matches between the phage and bacteria we
...
not an important concern in this analysis.

We start by finding all 15mer hits between the bacteria and the phages,
...
overlapping, hits must have come from two longer matching sequences.

To find all 15-mers in common between the phage and bacteria, sort and
...
direction, and then combine those matches.
...
to find the longest match per phage, tabulate the RefSeq IDs, and score
the matches we use:
...
Similar to codon usage below, we calculate the G+C/G+C+A+T content of
the coding regions. This gives us a simple one dimensional matrix that
we can use to map phages to hosts.
...
To count the bacterial GC content and limit that to complete bacterial
sequences we used:
...
Now we have two files: ``complete_bacteria_gc.tsv`` and ``phage.gc.tsv``
and we use pairwise `Euclidean
distance <http://en.wikipedia.org/wiki/Euclidean_distance>`__ to find
the closest organisms, convert them to taxa, and score the results.
...
We count the codon profiles in both host and bacteria, and then
calculate the `Euclidean
distance <http://en.wikipedia.org/wiki/Euclidean_distance>`__ using
`codon\_distance.py <code/codon_distance.py>`__ to identify the closest
host for each phage:
...
We used `Jellyfish <http://www.genome.umd.edu/jellyfish.html>`__ to
count *k-*\ mers in the DNA sequences. The longest phage sequence is
358,663 bp, so we used a hash size of 400,000 to keep the whole thing
(or most of it) in memory.
...
and then we modify it to count all k-mers between 3 and 20. That runs on
a node of the cluster in a few minutes.

We then combine those outputs to a single file using
`combineKmerCounts.py <code/combineKmerCounts.py>`__:
...
This generates a single tsv file for every genome that we are going to
use in the analysis.

Finally, we just make sure we only include genomes that we are
interested in for our analysis.
...
We need to predict the presence of bacteria and phage in all of the
metagenomes that we have downloaded. We have 3,205 metagenomes that we
have downloaded from `MG-RAST <http://metagenomics.anl.gov/>`__, however
...
We start with `FOCUS <http://edwards.sdsu.edu/FOCUS/>`__, but we need to
...
Now we can run all the bacterial genomes using focus. This script makes
...
Since there are 3025 files in mg-rast.txt, we use qsub to run this
command ``qsub  -cwd -S /bin/bash -t 1-3025:1 -o sge -e sge ./focus.sh``

This takes just a few hours to predict all the bacteria present in all
...
``python ~/bioinformatics/phage_host/focus_parse.py focus_NC2 > focus_bacteria_predictions.tsv``

Predicting phage sequences
--------------------------

We have already run megablast for all phages against all metagenomes for
another project, and so we just use this as our input.

The code below makes two tables called ``raw_phage_mg_counts.tsv`` and
``normalized_phage_mg_counts.tsv`` listing the presence of phages in
...
use (longer metagenomes have more hits, after all).

::

        python2.7 PhageHosts/code/count_phage_mg_hits.py 2> err

Our computations produce two files, ``normalized_phage_mg_counts.tsv``
and ``focus_bacteria_predictions.tsv`` that we need to compare. To do
so, we calculate Pearson's correlation between genomes and phages,
convert those to taxonomy IDs and then score them.
...
We have also developed code to compare the scores using Euclidean
...
calculations, computing the taxonomy IDs and scoring the results.
